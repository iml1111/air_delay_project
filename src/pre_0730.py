...
def proc(filename = 'AFSNT.csv'):
	...
	# 지연에서 이상치 데이터 다 빼기
	df = df.loc[
		((df['DLY'] == 'Y') & (df['DRR'].notnull())) |
		((df['DLY'] == 'N') & (df['DRR'].isnull()))
	]
	...
	df = df[ (df["DRR"] == "C02") | (df['DRR'].isnull())]
	...
	df = df[df["REG"] != "null"]
	# FLT 편명 빈도수가 상위 500개 이하 날리기(500개)
	temp = df.groupby("FLT").size().sort_values(ascending = False).head(500)
	reg_list = list(temp.index)

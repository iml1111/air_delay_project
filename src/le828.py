...
def test_proc(df, est = 100):
	#### Train, Valid Set 준비하기
	split = StratifiedShuffleSplit(n_splits = 1, test_size = 0.2, random_state=42)
	df1 = df.loc[df["DLY"].notnull()]
	...
	print(Y_pred)
	print("예측값 길이",len(Y_pred))
	#print(val_y)
	print(random_forest)
	y_score = random_forest.predict_proba(val_x)
	print(y_score)
	print("예측 스코어 길이",len(y_score))
	print('saving model..')
...
def l_proc(df, est = 100):
	...
	return roc_auc_score(val_y, y_score[:,1])
